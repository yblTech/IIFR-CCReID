...
def main(config,dpath):
    ...
    print(dpath)
    # Build model
    model, classifier, discriminator, genertor = build_model(config, dataset.num_train_pids, dataset.num_train_clothes)             
    # Build loss    
    ...
    parameters = list(model.parameters()) + list(classifier.parameters()) + list(genertor.parameters())
    parameters2 = list(discriminator.parameters())
    lr_min = 3.5e-6
    lr_max = 3.5e-4
    step_size_up = 10
    ...
    if config.TRAIN.OPTIMIZER.NAME == 'adam':
        ...
        optimizer_cc = optim.Adam(parameters2, lr=config.TRAIN.OPTIMIZER.LR, 
                               weight_decay=config.TRAIN.OPTIMIZER.WEIGHT_DECAY)
    ...
    discriminator = discriminator.cuda(local_rank) 
    genertor = genertor.cuda(local_rank)
    ...
    if config.TRAIN.AMP:
        [model, classifier,genertor], optimizer = amp.initialize([model, classifier,genertor], optimizer, opt_level="O1")
        [discriminator], optimizer_cc = amp.initialize([discriminator], optimizer_cc, opt_level="O1")
    ...
    genertor =  nn.parallel.DistributedDataParallel(genertor, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True)
    if config.EVAL_MODE:
    ...
    for epoch in range(start_epoch, config.TRAIN.MAX_EPOCH):
        ...
        train_adv(config, epoch, model, genertor, discriminator, classifier, dataset.num_train_pids,criterion_cla, criterion_cal, 
        ...
        if (epoch+1) > config.TEST.START_EVAL and config.TEST.EVAL_STEP > 0 or epoch == 4 or epoch == 9 or epoch == 14 or epoch == 0:
            ...
            if config.DATA.DATASET == 'prcc':
                rank1 = test_prcc(config,model, queryloader_same, queryloader_diff, galleryloader, dataset)
...
if __name__ == '__main__':
    ...
    source_file=["main.py","test.py","train.py",
    yp, dp]
